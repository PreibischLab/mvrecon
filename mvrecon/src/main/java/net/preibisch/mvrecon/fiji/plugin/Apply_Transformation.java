...
import net.preibisch.mvrecon.fiji.plugin.ApplyTransformationCore;

public class Apply_Transformation extends ApplyTransformationCore implements PlugIn
{
			...
			final ApplyParameters params )
	{
		...
		final ListImg< ModelLink > img = ApplyTransformationCore.createTable(
				data, viewIds,
		...
		for ( final ViewId viewId : viewIds )
		{
			...
			ApplyTransformationCore.locationForViewDescription( l, vd, mapT, mapC, mapI, mapA );
		...
		final int numEntries = ApplyTransformationCore.numEntries( img );
	...
	public void applyModels( final SpimData spimData, final double minResolution, final int applyTo, final Map< ViewDescription, Pair< double[], String > > modelLinks )
	{
		for ( final ViewDescription vd : modelLinks.keySet() )
		{
			...
			if ( applyTo == 0 )
			{
				...
				ApplyTransformationCore.setModelToIdentity( spimData, vd );
			}
			else if ( applyTo == 1 )
			{
				...
				ApplyTransformationCore.setModelToCalibration( spimData, vd, minResolution );
			}
			...
			if ( v != null )
			{
				...
				ApplyTransformationCore.preConcatenateTransform( spimData, vd, model, "Manually defined transformation (" + modelDesc + ")" );
			}
		}
	}
