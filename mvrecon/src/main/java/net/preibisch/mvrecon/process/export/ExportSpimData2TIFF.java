...
import net.preibisch.mvrecon.fiji.plugin.resave.ResaveTiffCore;
...
public class ExportSpimData2TIFF implements ImgExport
{
	...
	{
		if ( ResaveTiffCore.defaultPath == null )
			ResaveTiffCore.defaultPath = "";
			...
			final File basePath )
	{
		...
		try
		{
			timepoints = new TimePointsPattern( ResaveTiffCore.listAllTimePoints( timepointsToProcess ) );
		}
		catch (ParseException e)
		{
			IOFunctions.println( "Automatically created list of timepoints failed to parse. This should not happen, really :) -- " + e );
			IOFunctions.println( "Here is the list: " + ResaveTiffCore.listAllTimePoints( timepointsToProcess ) );
			e.printStackTrace();
			return null;
		}
