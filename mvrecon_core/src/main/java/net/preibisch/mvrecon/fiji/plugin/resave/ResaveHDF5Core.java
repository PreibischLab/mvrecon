...
import net.preibisch.mvrecon.fiji.plugin.resave.GenericResaveHDF5Core.Parameters;
import net.preibisch.mvrecon.fiji.spimdata.SpimData2;
import net.preibisch.mvrecon.fiji.spimdata.interestpoints.ViewInterestPointLists;
import net.preibisch.mvrecon.fiji.spimdata.interestpoints.ViewInterestPoints;

public class ResaveHDF5Core {
	...
	public static SpimData2 reduceSpimData2( final SpimData2 oldSpimData, final List< ViewId > viewIds )
	{
		...
		try
		{
			timepoints = new TimePointsPattern( ResaveTiffCore.listAllTimePoints( SpimData2.getAllTimePointsSorted( oldSpimData, viewIds ) ) );
		}
		catch (ParseException e)
		{
			IOFunctions.println( "Automatically created list of timepoints failed to parse. This should not happen, really :) -- " + e );
			IOFunctions.println( "Here is the list: " + ResaveTiffCore.listAllTimePoints( SpimData2.getAllTimePointsSorted( oldSpimData, viewIds ) ) );
			e.printStackTrace();
			return null;
		}
			...
			final boolean useRightAway )
	{
		...
		final SpimData2 newSpimData = ResaveTiffCore.assemblePartialSpimData2( spimData, viewIds, params.seqFile.getParentFile(), filesToCopy );
		final ArrayList< Partition > partitions = GenericResaveHDF5Core.getPartitions( newSpimData, params );
